---
title: Pan Plugin
description: Enables users to scroll the document by clicking and dragging with a hand tool.
searchable: true
---
...
The plugin works by registering a "pan" interaction mode. You can build UI to toggle this mode on and off using the `usePan` hook.

### Building a Pan Tool Button

The `usePan` hook provides the `isPanning` state and a `provides` object with methods to control the pan mode. The `isPanning` boolean is perfect for providing visual feedback in your UI, such as highlighting the tool's button when it is active.
...
import { PDFViewer } from '../code-examples/pan-example';
